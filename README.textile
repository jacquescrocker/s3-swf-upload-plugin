...
4. Configure your s3 settings in the generated <code>config/initializers/s3_swf_upload.rb</code> file.

5. Run the setup rake task (on all environments)

<pre><code>$ rake s3_swf_upload:remote_setup
</code></pre>
...
7. To achieve basic functionality, the only other thing you need is something to call the <code>startUploading</code> function.  For example:
...
You'll want to add the view helper to an initialization file: <code>require 'view_helpers'</code>
...
2. If the <code>startUploading</code> call is made after calling <code>stopUploading</code>, only the first file in the queue is successfully uploaded.
